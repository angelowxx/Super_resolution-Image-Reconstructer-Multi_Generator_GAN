...
def calculate_ssim(img1, img2):
    img1_np = np.array(img1.squeeze(0).cpu(), dtype=np.float32)
    img2_np = np.array(img2.squeeze(0).cpu(), dtype=np.float32)
...
class ReconstructionLoss(nn.Module):
    def __init__(self):
        ...
        self.diff_kernel = torch.tensor([[-1/8, -1/8, -1/8],
                                         [-1/8, 1, -1/8],
    ...
    def high_pass_filter(self, images):
        ...
        edges = torch.clamp(self.normalize(edges, 1, 0.2), 0, 2)
    ...
    def total_variation_loss(self, image, reversed_edges):
        ...
        tv_loss = torch.relu(torch.mean(diff))
