...
def train_example(rank, world_size, num_epochs):
    ...
    train_loader = torch.utils.data.DataLoader(train_data, batch_size=4, sampler=sampler, num_workers=0)
    ...
    val_loader = torch.utils.data.DataLoader(val_data, batch_size=4, sampler=sampler_val, num_workers=0)
...
def train_image_finger_print(image_finger_print, generator, hr_imgs, d_optimizer):
    ...
    d_loss = uniformity_loss(fake_preds)
